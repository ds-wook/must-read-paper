...
class BaseModel(ABC):
    ...
    def run_cv_training(self, X: pd.DataFrame, y: pd.Series) -> None:
        ...
        for fold, (train_idx, valid_idx) in enumerate(iterable=kfold.split(X=X, y=y), start=1):
            ...
            scores.add_row([fold, f1_score(y_valid, (oof_preds[valid_idx] > 0.5).astype(np.int8), average="macro")])

        del X_train, X_valid, y_train, y_valid, model
        gc.collect()

        print(f"{scores.get_string()}")
        print(f"CV Score: {f1_score(y, (oof_preds > 0.5).astype(np.int8), average='macro')}")
